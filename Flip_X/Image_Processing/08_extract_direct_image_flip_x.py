...
def build_approach_1(cover=None, wm=None):
    ...
    # Inverse classical mapping for Flip X: y -> 1 - y
    for i in range(n):
        qc.x(q_y[i])
...
if __name__ == "__main__":
    ...
    try:
        ...
        fig1.savefig('../../outputs/Image_2_Direct_Extraction_Circuit_FlipX.png', bbox_inches="tight", dpi=300)
    except Exception as e:
        print(f'Notice: Circuit diagram too large to draw/save ({type(e).__name__}). Skipping circuit image.')
        
    wm1 = simulate_and_reconstruct(qc1, n, size, "Approach 1 Direct Flip X")
    print("Approach 1 Match:", np.array_equal(wm1, original_wm))

    cv2.imwrite('../../outputs/Image_3_Output_Watermark_Direct_FlipX.png', wm1 * 255)
    print('Saved extracted watermark to outputs/Image_3_Output_Watermark_Direct_FlipX.png')
